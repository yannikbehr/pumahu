...
class MerweScaledSigmaPoints(object):
    ...
    def sigma_points(self, x, P):
        ...
        return (self.weights, sigmas)

    def constrained_sigma_points(self, x, P, e, d):
        """
        Computes the interval-constrained sigma points based on [1]
        for an unscented Kalman filter given the mean (x) and
        covariance(P) of the filter.
        Returns tuple of the sigma points and weights.
        ...
        e : scalar, or np.array
            Upper bound interval constraints

        d : scalar, or np.array
            Lower bound interval constraints
        ...
        [1] Teixeira et al., Journal of Process Control, 2010
        """
        if self.n != np.size(x):
            raise ValueError("expected size(x) {}, but size is {}".format(
                self.n, np.size(x)))
        ...
        if np.isscalar(P):
            ...
            P = np.atleast_2d(P)
        # add some small delta to make sure that
        # weights don't become zero
        jitter = 0.01
        x = np.where(x < e, x, e - (e-d)*jitter)
        x = np.where(x > d, x, d + (e-d)*jitter)
        lambda_ = self.alpha**2 * (n + self.kappa) - n
        U = self.sqrt(P).T
        S = np.concatenate((U, -U), axis=1)
        fact = np.sqrt(lambda_ + n)
        theta = np.full((n, 2*n), fact)
        ll = d - x
        ul = e - x
        for i in range(n):
            for j in range(2*n):
                if S[i, j] > 0:
                    theta[i, j] = min(fact, ul[i]/S[i, j])
                elif S[i, j] < 0:
                    theta[i, j] = min(fact, ll[i]/S[i, j])

        nfact = np.min(theta, axis=0)

        # Compute new weights
        ak = (2*lambda_ - 1)/(2*(n+lambda_)*(nfact.sum() - (2*n + 1)*fact))
        bk = 1./(2*(n+lambda_)) - (2*lambda_-1)/((2*fact)*(nfact.sum() - (2*n+1)*fact))

        sigmas = np.zeros((2*n+1, n))
        sigmas[0] = x
        weights = np.zeros((2, 2*n+1))
        for k in range(n):
            # pylint: disable=bad-whitespace
            sigmas[k+1] = self.subtract(x, nfact[k]*-U[:, k])
            sigmas[n+k+1] = self.subtract(x, nfact[n+k]*U[:, k])
            weights[:, k+1] = ak*nfact[k] + bk
            weights[:, n+k+1] = ak*nfact[n+k] + bk
        weights[0, 0] = bk
        weights[1, 0] = bk + (1 - self.alpha**2 + self.beta)
    ...
    def _compute_weights(self):
        ...
        self.weights = np.zeros((2, 2*n+1))
        self.weights[0] = self.Wm
        self.weights[1] = self.Wc
